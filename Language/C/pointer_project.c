...
// 물고기가 6마리가 있다.
// 이들을 어항에 살고 있는데, 사막이다.
// 사막이 너무 덥고 건조해서 물이 아주 빨리 증발을 한다.
// 물이 다 증발하기 전에 부지런히 어항에 물을 줘 물고기를 살려야한다.
// 물고기는 시간이 지날 수록 점점 커져서 나중에 구워먹을 것이다.
int level;
int arrayFish[6];
int * cursor;
...
int main_pointer_project(void)
{
    ...
    while(1)
    {
        ...
        // 사용자가 입력한 어항에 물을 준다
        // 1. 어항의 물이 0이면? 물을 주지 않는다. 이미 물고기가 죽은 것이다.
        if(cursor[num -1] <= 0)
        {
            printf("%d 번 물고기는 이미 죽었습니다. 물을 주지 않습니다\n", num);
        }
        // 2. 어항의 물이 0 이 아닌 경우? 물을 준다 ! 100 을넘지 않는 지 체크
        ...
        // 레벨업을 할 건지 확인 (레벨업은 20초마다 한번씩 수행행
        ...
        // 10 초 -> 15 초 (5초 : prevElapsedTime) -> 25초 (10초...?)
    }
